...
{
	$query="SELECT `numero`,`annee`,`prenom_ar`,`nom_ar` FROM `acte_deces` WHERE `numero`=? AND `annee`=?";
	$pdoResult = $pdoConnect->prepare($query);
	$pdoResult->execute(array($numero,$annee));
}
if(trim($numero)=="")
{
	$query="SELECT `numero`,`annee`,`prenom_ar`,`nom_ar` FROM `acte_deces` WHERE `annee`=?";
	$pdoResult = $pdoConnect->prepare($query);
...
if(trim($annee)=="")
{
	$query="SELECT `numero`,`annee`,`prenom_ar`,`nom_ar` FROM `acte_deces` WHERE `numero`=?";
	$pdoResult = $pdoConnect->prepare($query);
