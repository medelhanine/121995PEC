...
{
	$query="SELECT `numero`,`annee`,`prenom_ar`,`nom_ar`,`validate` FROM `sdeadtable` WHERE `nom_ar`=? AND `nom`=?";
	$pdoResult = $pdoConnect->prepare($query);
	$pdoResult->execute(array($nameAr,$nameFr));
}
if(trim($nameAr)=="")
{
	$query="SELECT `numero`,`annee`,`prenom_ar`,`nom_ar`,`validate` FROM `sdeadtable` WHERE `nom`=?";
	$pdoResult = $pdoConnect->prepare($query);
...
if(trim($nameFr)=="")
{
	$query="SELECT `numero`,`annee`,`prenom_ar`,`nom_ar`,`validate` FROM `sdeadtable` WHERE `nom_ar`=?";
	$pdoResult = $pdoConnect->prepare($query);
