...
{
	$query="SELECT `numero`,`annee`,`prenom_deces`,`nom_deces`,`dateInsert` FROM `tasrih_deces` WHERE `nom_deces`=? AND `nom_deces_fr`=?";
	$pdoResult = $pdoConnect->prepare($query);
	$pdoResult->execute(array($nameAr,$nameFr));
}
if(trim($nameAr)=="")
{
	$query="SELECT `numero`,`annee`,`prenom_deces`,`nom_deces`,`dateInsert` FROM `tasrih_deces` WHERE `nom_deces_fr`=?";
	$pdoResult = $pdoConnect->prepare($query);
...
if(trim($nameFr)=="")
{
	$query="SELECT `numero`,`annee`,`prenom_deces`,`nom_deces`,`dateInsert` FROM `tasrih_deces` WHERE `nom_deces`=?";
	$pdoResult = $pdoConnect->prepare($query);
		...
		<tr>
			<td>
				 <div style="width:860px; height:200px; overflow:auto;">
					 <table class="table table-hover " width="840" id="tableTasrihDeces">
	<?php
...
?>
   ...
   <tr class="droid-arabic-kufi">
    ...
    <td style="text-align: center;color:green;"><?php echo $row["prenom_deces"] ?></td>
    <td style="text-align: center;color:green;"><?php echo $row["nom_deces"] ?></td>
  ...
  <?php
	...
	<tr class="droid-arabic-kufi">
	 ...
	 <td style="text-align: center;"><?php echo $row["prenom_deces"] ?></td>
	 <td style="text-align: center;"><?php echo $row["nom_deces"] ?></td>
